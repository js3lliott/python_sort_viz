...
class Algorithm:
    def __init__(self, name):
        self.array = random.sample(range(256), 256) # random array of size 512
        self.name = name

    def update_display(self, swap1=None, swap2=None):
        import visualizer
        visualizer.update(self, swap1, swap2) # pass the indexes to be swapped in the visualizer
...
class InsertionSort(Algorithm):
    ...
    def algorithm(self):
        for i in range(len(self.array)):
            curr = self.array[i]
            idx = i
            while idx > 0 and self.array[idx - 1] > curr:
